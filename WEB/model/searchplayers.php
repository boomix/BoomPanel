<?php
    ...
    if(isset($_GET['search'], $_GET['submit']))
    {
        ...
        $LeftJoin = "FROM bp_players p 
                  LEFT JOIN bp_players_ip pip ON p.id = pip.pid 
                  LEFT JOIN bp_players_username purn ON p.id = purn.pid 
                  LEFT JOIN bp_players_online pol ON p.id = pol.pid
                  LEFT JOIN bp_countries bctr ON p.country = bctr.country_code ".$searchQuery;

        //Run search
        $SearchUsers    = $db->select("SELECT *, SUM(TIMESTAMPDIFF(MINUTE, connected, disconnected)) AS timeonline, MAX(pol.connected) AS last_online, MIN(pol.connected) AS first_online ".$LeftJoin." GROUP BY steamid, id DESC LIMIT ".$limit, $searchArray);
        ...
        echo_dev ("SELECT *, SUM(TIMESTAMPDIFF(MINUTE, connected, disconnected)) AS timeonline, MAX(pol.connected) AS last_online, MIN(pol.connected) AS first_online ".$LeftJoin." GROUP BY steamid, id DESC LIMIT ".$limit);
