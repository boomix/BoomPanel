<?php
    ...
    if(isset($_GET['search'], $_GET['submit']))
    {
        ...
        //Run search
        $SearchUsers    = $db->select("SELECT *, SUM(TIMESTAMPDIFF(MINUTE, connected, disconnected)) AS timeonline, MAX(pol.connected) AS last_online, MIN(pol.connected) AS first_online ".$LeftJoin." GROUP BY steamid, p.id DESC LIMIT ".$limit, $searchArray);
