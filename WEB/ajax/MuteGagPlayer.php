...
if(isset($_POST['steamid'], $_POST['serverName'], $_POST['reason']))
{
    ...
    $type       = (empty($_POST['mgtype']) || $_POST['mgtype'] < 0 || $_POST['mgtype'] > 2) ? 0 : intval($_POST['mgtype']);
    ...
    $db->query("INSERT INTO bp_mutegag (`pid`, `sid`, `aid`, `reason`, `length`, `mgtype`) VALUES (:pid, :sid, :aid, :reason, :length, :mgtype)", $data);
    ...
    try {
        ...
        echo 'sm_BPmutegagadd "'.$steamid.'" "'.$type.'" "'.$reason.'" "'.$length.'"';
        $data = $Query->Rcon('sm_BPmutegagadd "'.$steamid.'" "'.$type.'" "'.$reason.'" "'.$length.'"');
